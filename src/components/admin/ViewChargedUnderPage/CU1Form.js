    ...
    return (
        <>
                ...
                {hasWriteAccessTo(View.ADMIN.name) && (
                    <Form.Item>
                        {editing ? (
                            ...
                            <Button onClick={() => setEditing(true)} icon={<EditOutlined />} style={{ width: 85 }}>
                                Edit
                            </Button>
                        )}
                    </Form.Item>
            ...
            <Form
                ...
                autoComplete='off'
                labelAlign='left'
                initialValues={{ ...chargedUnder }}
            >
                <Form.Item label='Company Name' name='name' rules={editing ? [REQUIRED] : []}>
                    {!editing ? <Typography>{chargedUnder.name || '-'}</Typography> : <Input />}
                </Form.Item>

                <Form.Item label='Address' name='address' rules={editing ? [REQUIRED] : []}>
                    {!editing ? <Typography>{chargedUnder.address || '-'}</Typography> : <Input />}
                </Form.Item>

                <Form.Item label='Shipping Address' name='shipping_address' rules={editing ? [REQUIRED] : []}>
                    {!editing ? <Typography>{chargedUnder.shipping_address || '-'}</Typography> : <Input />}
                </Form.Item>

                <Form.Item label='Contact Number' name='contact_number' rules={editing ? [REQUIRED, minLength(8)] : []}>
                    {!editing ? <Typography>{chargedUnder.contact_number || '-'}</Typography> : <Input />}
                </Form.Item>

                <Form.Item label='Registration Number' name='registration_number'>
                    {!editing ? <Typography>{chargedUnder.registration_number || '-'}</Typography> : <Input />}
                </Form.Item>

                <Form.Item label='GST Rate' name='gst_rate'>
                    {!editing ? (
                        <Typography>
                            {chargedUnder.gst_rate && chargedUnder.gst_rate !== 0 ? `${chargedUnder.gst_rate}%` : '-'}
                        </Typography>
                    ) : (
